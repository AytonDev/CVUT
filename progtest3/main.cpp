...
vector<dateValue> dates;
void Push(dateValue date) {
...
public:
    ...
    CDate(int year, int month, int day) {
        ...
        if(m_leapYear == false &&month == 2 &&  day >= 29){
             throw InvalidDateException();
        }
        m_year = year;
        m_month = month;
        m_days = day;
        m_date = true;
       // m_days += DateToDays(year,month, day);
       
    }

     // constructor
    CDate(int year, int month, int day, bool tests) {
        if(day !=0)
            m_days = day;
        else if(month != 0)
            m_month = month;
    ...
    CDate operator + (CDate const &date)  {
        if(!m_date && !date.m_date){
           if( dates.empty()){
             Push((dateValue{m_year,m_month,m_day}));
              Push((dateValue{date.m_year,date.m_month,date.m_days}));
           }else {
               Push((dateValue{date.m_year,date.m_month,date.m_days}));
           }
      ...
      if(date.m_year != 0) {
            ...
            return  CDate(m_year+ date.m_year, m_month, m_days);
          
            
        }if(date.m_month !=0){
            if(date.m_month >= 12){  
                int years =  date.m_month /12;
                int months = date.m_month % 12;
                months += m_month;
                if(months > 12){
                    years++;
                    months = months %12;
                ...
                int months =m_month + date.m_month;
                int years=0;
                if(months > 12){
                    years++;
                    months = months %12;
        ...
        }else if(date.m_days !=0) {

            int days = DateToDays(m_year, m_month, m_days);
            int year=0, month=0, day =0;
           
            DaysToDate(days+date.m_days,year, month, day);
           // cout << year << "::" << month << "::" << day <<endl;
          
                return  CDate(year, month, day);
        }
        else {
             return  CDate(m_year, m_month, m_day);
        }
    }
     ...
     CDate operator - (CDate const &date)  {
       ...
       if(date.m_year != 0) {
            ...
            if(date.m_month >= 12){  
                ...
                return  CDate(m_year-years,12- months, m_days);
               
            }else  {
                int months; 
                if(m_month > date.m_month){
                    months =m_month - date.m_month;
                   
                        return  CDate(m_year, months, m_days);
                   
                }
                else {
                    months = 12+m_month-date.m_month;
                    
                        return  CDate(m_year-1, months, m_days);
                    
                }
                 
            }
        }else if(date.m_days !=0) {
           // cout << m_year << " " <<  m_month << " " <<  m_days <<endl;
            int days = DateToDays(m_year, m_month, m_days) - date.m_days;
            int year=0, month=0, day =0;
    ...
    CDate operator +=(CDate const &date){
        ...
        cout << m_days<< " " << m_month << " "<< m_year <<endl;
        for(int i=0; i < dates.size(); ++i){
           if(dates[i].year !=0){
               m_year += dates[i].year;
               if(!leapYearCalculator(m_year) && m_month == 2 && m_day>28){
                   throw InvalidDateException();
                   
               }
           }else if(dates[i].month !=0){
               if(m_month + dates[i].month >0 && m_month + dates[i].month <=12 ){
                   m_month = m_month +dates[i].month;
               }else if( m_month + dates[i].month >0) {
                   int months =dates[i].month %12;
                   int years = dates[i].month /12;
...
private:
    // todo
    int m_year=0, m_month=0, m_day=0, m_days=0;
   
    bool m_leapYear, m_date;
...
CDate Year(int year){
    return CDate(year,0,0,true);
}
CDate Month(int month){
    return CDate(0,month,0,true);
}
CDate Day(int day){
    return CDate(0,0,day, true);
}
